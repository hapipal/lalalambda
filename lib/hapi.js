...
exports.plugin = {
    ...
    register(server, { lambdaify }) {
        ...
        if (lambdaify) {
            ...
            Hoek.assert(!rootState.lambdaify, `Lalalambda's lambdaify option can only be specified once.`);
...
internals.lambdaifyServer = (server, config) => {
    ...
    Hoek.assert(lambdaConfig.id, `Lalalambda's lambdaize registration option must be configured with an id.`);
